...
Let $z \in \mathcal{Z}$ be an open problem in the zeitgeist, $E_\mathcal{D}$ be a semantic encoder trained over a discipline $\mathcal{D}$, and $s$ be a semantic similarity score. Then $S_\mathcal{I}(z) := \left[s(E_\mathcal{D}(z), E_\mathcal{D}(d)) ~ | ~ d \in \mathcal{I}\right]$ is the list of similarity scores between an individual's ideas and an open problem, sorted in descending order. The probability that an individual searches over an idea $z' \in S_\mathcal{I}(z)$ is proportional to the similarity score $s(E_\mathcal{D}(z), E_\mathcal{D}(z'))$.

### Novelty, Utility, and Surprise in the Scientific Creativity Setting

The difficulty of most open problems is that their solutions are non-obvious (or "surprising") with respect to that discipline's body of knowledge $\mathcal{D}$. Otherwise, experts of that discipline would have already solved it. Given an open problem $z \in \mathcal{Z}$ and an idea $d$ or permutation of ideas $[d_1, \dots, d_k]$ that solves problem $z$, we can take non-obviousness to mean that $E_\mathcal{D}(z, d)$ is small or $E_\mathcal{D}(z, d_j)$ is small for all $j \in [k]$. 

Note, however, that within the encompassing scientific enterprise $\mathcal{E}$, a creative idea may cease to be surprising because $E_\mathcal{E}(z, d)$ may be large or $E_\mathcal{E}(z, d_j)$ may not be small for all $j \in [k]$.
