...
Let $z \in \mathcal{Z}$ be an open problem in the zeitgeist, $E_\mathcal{D}$ be a semantic encoder trained over a discipline $\mathcal{D}$, and $s$ be a semantic similarity score. Then $S_\mathcal{I}(z) := \left[s(E_\mathcal{D}(z), E_\mathcal{D}(d)) ~ | ~ d \in \mathcal{I}\right]$ is the list of similarity scores between an individual's ideas and an open problem, sorted in descending order. The **semantic similarity hypothesis** states that the probability an idea $z' \in S_\mathcal{I}(z)$ is chosen to be permuted or evaluated as a solution for $z$ is proportional to the similarity score $s(E_\mathcal{D}(z), E_\mathcal{D}(z'))$.
